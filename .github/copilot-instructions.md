...
### LaTeX Architecture
- **Document class**: `src/spellcard.cls` - Custom class with draft/final modes (controls cardify loading)
- **Entry point**: `src/spellcards.tex` - Main document using `\documentclass{spellcard}` (final mode by default)
- **Modern modular package**: `src/spellcards.sty` - Package loader for 7 feature modules
- **Package modules**: `src/spellcards/*.sty` - Modular expl3 implementation
  - `core.sty` - Variables, constants, messages
  - `properties.sty` - Spell data management
  - `deck.sty` - Deck organization and labels
  - `level-markers.sty` - Spell level indicators
  - `qr-code.sty` - QR code positioning (renamed to avoid system package conflict)
  - `info-table.sty` - Spell attribute table rendering
  - `content-layout.sty` - Card structure orchestrator
- **Cardify layout**: `src/cardify.tex` - A6-on-A4 print layout (loaded conditionally in final mode)
...
- Compile with `latexmk -pv- -pdf src/test-expl3.tex </dev/null`
- Verify output: `pdftotext src/out/test-expl3.pdf - | grep "expected pattern"`
- Update tests when adding features

**Testing Requirements by File Type**:

**When modifying modular package files** (`src/spellcards/*.sty`):
1. ✅ **MUST compile main document**: `latexmk -pv- -pdf src/spellcards.tex </dev/null`
2. ✅ **MUST verify exit code 0**: `echo $?`
3. ✅ **SHOULD verify page count unchanged**: `pdfinfo src/out/spellcards.pdf | grep Pages` (compare with before)
4. ✅ **SHOULD run specific feature test** if it exists:
   - `src/spellcards/core.sty` → compile `src/test-expl3.tex`
   - `src/spellcards/properties.sty` → compile `src/test-expl3.tex`
   - `src/spellcards/deck.sty` → compile `src/test-expl3.tex`
   - `src/spellcards/level-markers.sty` → compile `src/test-expl3.tex`
   - `src/spellcards/qr-code.sty` → compile `src/test-expl3.tex`
   - `src/spellcards/info-table.sty` → compile `src/test-table-rendering.tex`
   - `src/spellcards/content-layout.sty` → compile `src/test-spell.tex`

**When modifying document class** (`src/spellcard.cls`):
1. ✅ **MUST test both modes**:
   - Draft mode: `\documentclass[draft]{spellcard}`
     - Compile: `latexmk -pv- -pdf src/spellcards.tex </dev/null`
     - Verify: Single-sided output, A4 page size, no cardify layout
   - Final mode: `\documentclass{spellcard}` (default, or `\documentclass[final]{spellcard}`)
     - Compile: `latexmk -pv- -pdf src/spellcards.tex </dev/null`
     - Verify: Double-sided output, A4 page size, cardified (4 cards per sheet)
2. ✅ **MUST verify exit code 0** for both modes
3. ✅ **SHOULD check page counts** match expected values for each mode

**When modifying cardify layout** (`src/cardify.tex`):
1. ✅ **MUST compile in final mode**: `\documentclass{spellcard}` (default)
2. ✅ **MUST verify cardified output**: Check PDF has 4 cards per A4 sheet
3. ✅ **MUST verify cutting guides**: Visual inspection of PDF
4. ✅ **SHOULD test without cardify**: Draft mode to verify no interference

**When modifying main document** (`src/spellcards.tex`):
1. ✅ **MUST compile**: `latexmk -pv- -pdf src/spellcards.tex </dev/null`
2. ✅ **MUST verify exit code 0**
3. ✅ **SHOULD verify page count unchanged**: Compare before/after

**General Test Pattern**:
```bash
# Compile with clean build
latexmk -C src/spellcards.tex >/dev/null 2>&1
latexmk -pv- -pdf src/spellcards.tex </dev/null
echo "Exit code: $?"

# Verify page count
pdfinfo src/out/spellcards.pdf | grep "Pages:"

# Check for errors (should be empty)
grep "^!" src/out/spellcards.log

# Check for warnings (should be empty)
grep "Warning" src/out/spellcards.log
```
- Create test documents for each package/feature
