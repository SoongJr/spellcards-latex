...
\newcounter{decknumber}
\setcounter{decknumber}{0}
%
% Variable to store the current deck name
\newcommand{\currentdeck}{}
%
% Command to start a new deck
\newcommand{\deckname}[1]{%
  \stepcounter{decknumber}%
  \renewcommand{\currentdeck}{#1}%
}
%
% Command to clear the current deck
\newcommand{\cleardeck}{%
  \renewcommand{\currentdeck}{}%
}
%
...
\newcommand{\drawdecklabel}[2]{%
  % #1 = deck name, #2 = deck number
  \ifthenelse{\equal{#1}{}}{}{%
    \begin{tikzpicture}[remember picture, overlay]%
      % Calculate horizontal position based on deck number
      % Position from left edge, with spacing for multiple decks
      \pgfmathparse{mod(#2-1,5)}% support up to 5 decks before wrapping
      \pgfmathsetmacro{\labelposition}{\fpeval{((1/6 * \pgfmathresult)) * \paperwidth} + 1.5cm}%
      ...
      \ifdefined\printermarginy%
        \node [font=\normalsize\itshape, anchor=south, xshift=\labelposition, yshift=-\printermarginy-2mm] at (current page.north west){#1};%
      \else%
        \node [font=\normalsize\itshape, anchor=south, xshift=\labelposition, yshift=-1cm] at (current page.north west){#1};%
      \fi%
    \end{tikzpicture}%
  }%
}
...
\newenvironment{spellcard}[3]{%
  ...
  \drawdecklabel{\currentdeck}{\value{decknumber}}%
