...
class SpellTabManager:
    ...
    def _create_spell_tab(self, class_name: str):
        ...
        level_combo, source_combo, search_entry = self._get_filter_widgets(filters_frame)
    ...
    def _create_filters_frame(self, parent_frame: ttk.Frame, class_name: str) -> ttk.LabelFrame:
        """Create filters frame for a class tab."""
        filters_frame = ttk.LabelFrame(parent_frame, text="Filters", padding=UIConfig.MAIN_PADDING)
        ...
        level_combo.bind('<<ComboboxSelected>>', lambda e: self._apply_filters(class_name))
        ...
        source_combo.bind('<<ComboboxSelected>>', lambda e: self._apply_filters(class_name))
    ...
    def _create_spells_list(self, parent_frame: ttk.Frame, class_name: str) -> ttk.Treeview:
        ...
        spells_tree.bind('<Button-1>', lambda e: self._toggle_spell_selection(e, spells_tree))
    ...
    def _setup_class_filters(self, class_name: str):
        ...
        levels = self.spell_filter.get_available_levels(self.data_loader.spells_df, class_name)
        class_data.level_combo['values'] = levels
        
        # Update source filter options
        sources = self.spell_filter.get_available_sources(self.data_loader.spells_df, class_name)
    ...
    def _apply_filters(self, class_name: str):
        ...
        if (self.data_loader.spells_df is None or 
            class_name not in self.spell_data):
            return
        ...
        filtered_df = self.spell_filter.filter_spells(
            ...
            class_name,
            level=class_data.level_var.get() if class_data.level_var.get() != "All" else None,
            source=class_data.source_var.get() if class_data.source_var.get() != "All" else None,
    ...
    def _show_spell_preview(self, spell_name: str, spell_data: pd.Series, class_name: str):
