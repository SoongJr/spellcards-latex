...
class ClassSelectionManager:
    ...
    def __init__(
        self, parent_frame: ttk.Frame, selection_callback: Callable[[Set[str]], None]
    ):
    ...
    def setup_class_sections(self, character_classes: list):
        ...
        for section_name, section_data in left_sections:
            self._create_class_section_in_frame(
                left_frame, section_name, section_data, row
            )
            row += 2
        ...
        for section_name, section_data in right_sections:
            self._create_class_section_in_frame(
                right_frame, section_name, section_data, row
            )
            row += 2

        # Add global selection buttons at the bottom
        self._create_global_selection_buttons(
            self.container_frame, max(len(left_sections), len(right_sections)) * 2
        )
    ...
    def _create_class_section_in_frame(
        ...
        section_obj = SectionData(
            classes=section_data["classes"], expanded=section_data["expanded"]
        )
        ...
        toggle_btn = ttk.Button(
            header_frame,
            text=(
                UIConfig.EXPAND_ICON if section_obj.expanded else UIConfig.COLLAPSE_ICON
            ),
            width=3,
        ...
        section_label = ttk.Label(
            header_frame, text=section_name, font=("TkDefaultFont", 9, "bold")
        )
        ...
        if section_obj.expanded:
            classes_frame.grid(
                row=row + 1, column=0, sticky=(tk.W, tk.E), padx=15, pady=(0, 5)
            )
    ...
    def _create_global_selection_buttons(
        self, container_frame: ttk.Frame, total_rows: int
    ):
        ...
        ttk.Button(
            inner_frame, text="Select All Classes", command=self.select_all_classes
        ).pack(side=tk.LEFT, padx=5)

        ttk.Button(
            inner_frame, text="Deselect All Classes", command=self.deselect_all_classes
        ).pack(side=tk.LEFT, padx=5)

        ttk.Button(
            inner_frame, text="Core Classes Only", command=self.select_core_classes
        ).pack(side=tk.LEFT, padx=5)
    ...
    def _toggle_section_selection(self, section_name: str):
        ...
        selected_count = sum(
            1
            for cls in classes
            if cls in self.class_vars and self.class_vars[cls].get()
        )
